...
export type TutorSidebarProps = {
  institutionName?: string | null;
};

export default function TutorSidebar({ institutionName }: TutorSidebarProps) {
  const pathname = usePathname();
  return (
    <div className="flex flex-col h-full">
      {institutionName ? (
        <p className="px-3 pt-2 text-xs text-muted-foreground truncate" title={institutionName ?? undefined}>
          {institutionName}
        </p>
      ) : null}
